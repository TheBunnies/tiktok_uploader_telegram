    ...
    teloxide::repls2::repl(bot, |message: Message, bot: AutoSend<Bot>| async move {
        ...
        if REGEX.is_match(message_text) {
            ...
            if let None = resp {
                ...
                log::info!("{} Rejected request `Tiktok link does not lead to anything`: {} From user: {} {}/{}", Local::now(), message_text, user.first_name, last_name, user.id.0);
            ...
            let user = message.from().unwrap();
            let last_name = user.last_name.to_owned().unwrap_or(String::new());
            log::info!("{} Now processing: {} From user: {} {}/{}", Local::now(), message_text, user.first_name, last_name, user.id.0);
